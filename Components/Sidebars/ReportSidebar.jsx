...
const ReportSidebar = ({ skillAnalysis, strengthPoints, weaknessesPoints }) => {
  const renderPoints = (points) => {
    if (!points) {
      return <MainText title={"N/A"} className="text-sm text-ui-textMuted" />;
    }
    return points.split("\n").map((point, idx) => {
      const cleanPoint = point.replace(/^\*\s*/, "").trim();
      if (!cleanPoint) return null;
      return <MainText key={idx} title={cleanPoint} className="text-sm text-ui-textMain dark:text-ui-muted leading-relaxed block" />;
    });
  };
        ...
        <div className="space-y-5">
          {skillAnalysis && skillAnalysis.length > 0 ? (
            map(skillAnalysis, (skill, idx) => {
              const skillScoreLabel = `${skill.score} %`;
              return (
                <div key={idx} className="space-y-1.5">
                  <div className="flex justify-between text-sm">
                    <MainText title={skill.skillName} className="font-medium text-ui-textMain dark:text-white" />
                    <MainText title={skillScoreLabel} className="text-ui-textMuted dark:text-ui-muted font-medium" />
                  </div>
                  <ProgressBar 
                    value={skill.score} 
                    showValue={false} 
            ...
            <MainText title={"No skills data available."} className="text-sm text-ui-textMuted dark:text-ui-muted" />
          )}
        </div>
      </div>
