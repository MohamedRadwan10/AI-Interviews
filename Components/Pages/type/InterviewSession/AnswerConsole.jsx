...
const AnswerConsole = ({ onSubmit, isSubmitting = false, isLastQuestion = false, isQuestionReady = true, questionType }) => {
  ...
  }, [activeTab, textAnswer]);

  const voiceTabContent = useMemo(() => {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full gap-6">
        ...
        <div className="flex gap-3">
          {status !== "recording" ? (
            <MainButton onClick={startRecording} title={"Start Recording"} className="px-6 py-2 bg-ui-textMain dark:bg-dark-primary-2 text-white rounded-xl" />
          ) : (
            <MainButton onClick={stopRecording} title={"Stop Recording"} className="px-6 py-2 bg-status-error text-white rounded-xl" />
          )}
          {mediaBlobUrl && <audio src={mediaBlobUrl} controls className="h-10 rounded-xl bg-light-primary" />}
        </div>
      </div>
    );
  }, [activeTab, status, mediaBlobUrl, startRecording, stopRecording]);
  ...
  return (
    <div className="bg-white dark:bg-dark-primary-4 rounded-3xl border border-ui-borderLight dark:border-ui-border shadow-sm overflow-hidden flex flex-col h-full">
      <div className="flex border-b border-ui-borderLight dark:border-ui-border p-2 gap-2 bg-light-primary/50 dark:bg-dark-primary-3/30">
        {map(tabs,(tab) => {
          const onTabClick = () => setActiveTab(tab.id);
          const isActive = activeTab === tab.id;
          const btnClass = `flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${isActive ? "bg-white dark:bg-dark-primary-4 text-brand-primary shadow-sm" : "text-ui-textMuted dark:text-ui-muted hover:text-ui-textMain dark:hover:text-white"}`;
          return (
            <MainButton
              key={tab.id}
              onClick={onTabClick}
