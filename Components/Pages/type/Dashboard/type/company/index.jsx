...
const CompanyDashboard = () => {
  ...
  if (loading) {
    return <Loading type="CompanyDashboard" />;
  }

  const handleViewApplicants = (job) => {
    navigateTo(`/intelliHire/job-applicants/${job.id}`);
  };

  const handleEditJob = (job) => {
    navigateTo(`/intelliHire/edit-job/${job.id}`);
  };

  const columns = getCompanyDashboardColumns(handleViewApplicants, handleEditJob, handleDeleteJob);
        ...
        <div className="flex flex-col gap-2">
          <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 bg-light-white dark:bg-dark-primary-3 p-6 rounded-3xl border border-ui-borderLight dark:border-ui-border">
            <MainText 
              ...
              className="text-xl font-bold text-ui-textMain dark:text-dark-white" 
            />
            <MainSearch
              value={searchTerm}
