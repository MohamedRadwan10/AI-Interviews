...
const ReportQuestionCard = (props) => {
  ...
  const difficultyStyles = useMemo(() => {
    ...
    if (diff === "hard") return "bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400";
    if (diff === "medium") return "bg-amber-50 text-amber-600 dark:bg-amber-900/20 dark:text-amber-400";
      ...
      >
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-1">
          ...
          <div className="flex items-center gap-3">
            ...
            {difficulty && (
              <MainText title={difficulty} className={`text-[10px] font-semibold tracking-wider uppercase px-2 py-0.5 rounded-full ${difficultyStyles}`} />
            )}
            {responseTime && (
